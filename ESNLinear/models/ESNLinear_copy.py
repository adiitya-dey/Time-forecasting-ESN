...
class ESN(nn.Module):
    ...
    def update_state(self, X):
        ...
        all_states = torch.empty(batch_size, self.reservoir_size, seq_len)
        for b in range(batch_size):
            for t in range(seq_len):
                self.state = self.get_state(X[b, :, t])
                all_states[b, :, t] = self.state.clone().detach().view(-1)
        return all_states
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.seasonal_esn = ESN(reservoir_size=100,
                             activation= nn.SELU(),
                             spectral_radius=0.95,
                             connectivity_rate=0.7,
                             input_size=1)
        
        self.trend_esn = ESN(reservoir_size=100,
                             activation= nn.SELU(),
                             spectral_radius=0.95,
                             connectivity_rate=0.7,
                             input_size=1)
    ...
    def reset(self):
        self.trend_esn.reset_states()
        self.seasonal_esn.reset_states()